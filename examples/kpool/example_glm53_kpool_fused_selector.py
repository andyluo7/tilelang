        ...
        with T.Kernel(num_rows, threads=threads) as row:
            ...
            if pool_count > group_topk:
                ...
                for tile in T.Pipelined(T.ceildiv(max_num_pools, block_n), num_stages=num_stages):
                    # Keep byte-vector stores within HIP's native 4-lane
                    # constructors; wider inferred uint8 vectors lower to an
                    # invalid make_uint4 call with one argument per byte.
                    for pool_lane, dim in T.Parallel(block_n, head_dim, coalesced_width=4):
                ...
                for radix_round in T.serial(4):
                    ...
                    if radix_round > 0:
                        ...
                        if tx == 0:
                            threshold_bin[0] = 0
                        T.sync_threads()
                        for scan in T.serial(T.ceildiv(max_num_pools, threads * 4)):
                            input_base = scan * threads * 4 + tx * 4
                            for item in T.serial(4):
                                input_idx = input_base + item
                                if input_idx >= pool_start and input_idx < pool_end:
...
def glm53_kpool_fused_select(
    ...
    *,
    ...
    Pass ``logits_workspace`` and ``out`` from a persistent framework-owned
    allocation when this function is captured in a CUDA/HIP graph.
    """
